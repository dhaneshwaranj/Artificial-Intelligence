...
class ProbabilityTests(unittest.TestCase):
    ...
    def test_games_network(self):
        ...
        self.assertTrue(B_node is not None, 'Team B node not initialized')
        B_dist = B_node.dist.table
        self.assertEqual(len(B_dist), 4, msg='Incorrect distribution size for Team B')
        test_prob = B_dist[0]
        test_prob2 = B_dist[2]
        self.assertEqual(int(test_prob * 100), 15, msg='Incorrect distribution for Team B')
        self.assertEqual(int(test_prob2 * 100), 30, msg='Incorrect distribution for Team B')

        C_node = games_net.get_node_by_name('C')
        self.assertTrue(C_node is not None, 'Team C node not initialized')
        C_dist = C_node.dist.table
        self.assertEqual(len(C_dist), 4, msg='Incorrect distribution size for Team C')
        test_prob = A_dist[0]
        test_prob2 = A_dist[2]
        self.assertEqual(int(test_prob * 100), 15, msg='Incorrect distribution for Team C')
        self.assertEqual(int(test_prob2 * 100), 30, msg='Incorrect distribution for Team C')
        ...
        self.assertTrue(BvC_node is not None, 'BvC node not initialized')

        BvC_dist = BvC_node.dist.table
        rows1, rows2, cols = BvC_dist.shape
        self.assertEqual(rows1, 4, msg='Incorrect match distribution size')
        self.assertEqual(rows2, 4, msg='Incorrect match distribution size')
        self.assertEqual(cols, 3, msg='Incorrect match distribution size')
        ...
        self.assertTrue(CvA_node is not None, 'CvA node not initialized')

        CvA_dist = CvA_node.dist.table
        rows1, rows2, cols = CvA_dist.shape
        self.assertEqual(rows1, 4, msg='Incorrect match distribution size')
        self.assertEqual(rows2, 4, msg='Incorrect match distribution size')
        self.assertEqual(cols, 3, msg='Incorrect match distribution size')
    ...
    def test_convergence(self):
        val = compare_sampling(get_game_network(), [], 0.001)
        print val

...
class DecisionTreePart2Tests(unittest.TestCase):
    ...
    def test_decision_tree_all_data(self):
        ...
        # accuracy = 0
        # for i in range(1):
        #     forest = dt.RandomForest(5, 5, 0.5, 0.5)
        #     forest.fit(self.train_features, self.train_classes)
        #     output = forest.classify(self.train_features)
        #     accuracy += float(sum([1 if output[i] == self.train_classes[i] else 0 for i in range(len(self.train_classes))])) / len(self.train_classes)
        #
        # print accuracy/1

        accuracy = 0
        for i in range(1):
            challenge = dt.ChallengeClassifier()
            challenge.fit(self.challenge_set_features, self.challenge_set_classes)
            output = challenge.classify(self.challenge_set_features)
            accuracy += float(sum([1 if output[i] == self.challenge_set_classes[i] else 0 for i in
                                   range(len(self.challenge_set_classes))])) / len(self.challenge_set_classes)

        print accuracy / 1
        print output
        print self.challenge_set_classes
